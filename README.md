...
| `WEB_ORIGIN` | prod | Exact web origin(s) for CORS + Better Auth `trustedOrigins`. No trailing slash. Comma-separated if needed. Live: `https://soft-spark.vercel.app`. Soft Spark Vercel preview hosts (`https://soft-spark-*.vercel.app`) are allowed without listing each PR URL. |
