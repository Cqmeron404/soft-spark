...
/** Align the credential hash to the public UI password when the email already exists. */
 ...
 * Safe on every API boot / Hobby cold start. Demo-scoped emails only.
 */
...
export async function maybeEnsureDemoUsers(deps: DemoUserSeedDeps): Promise<DemoUserSeedResult | undefined> {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error(`soft-spark-api: demo user seed failed (${message})`);
    return undefined;
  }
}
